...
class SpreadsheetSnippetsTest extends BaseTestCase {
  ...
  public function testBatchUpdate() {
    ...
    $response = $this->snippets->batchUpdate($id, 'New Title', 'Hello',
        'Goodbye');
    ...
    $this->assertEquals($findReplaceResponse->getValuesChanged(), 100,
  ...
  public function testUpdateValues() {
    ...
    $this->assertEquals(2, $result->getUpdatedRows(),
        'Wrong number of rows updated.');
    $this->assertEquals(2, $result->getUpdatedColumns(),
        'Wrong number of columns updated.');
    $this->assertEquals(4, $result->getUpdatedCells(),
        'Wrong number of cells updated.');
  }
  ...
  public function testBatchUpdateValues() {
    ...
    $this->assertEquals(1, count($result->getResponses()),
        'Wrong number of ranges updated.');
    $this->assertEquals(2, $result->getTotalUpdatedRows(),
        'Wrong number of rows updated.');
    $this->assertEquals(2, $result->getTotalUpdatedColumns(),
        'Wrong number of columns updated.');
    $this->assertEquals(4, $result->getTotalUpdatedCells(),
        'Wrong number of cells updated.');
  }
  ...
  public function testAppendValues() {
    ...
    $this->assertNotNull($result, 'No result returned.');
    $this->assertEquals('Sheet1!A1:J10', $result->getTableRange(),
        'Wrong table range appended to.');
    $updates = $result->getUpdates();
    $this->assertEquals(2, $updates->getUpdatedRows(),
        'Wrong number of rows appended.');
    $this->assertEquals(2, $updates->getUpdatedColumns(),
        'Wrong number of columns appended.');
    $this->assertEquals(4, $updates->getUpdatedCells(),
        'Wrong number of cells appended.');
  }
